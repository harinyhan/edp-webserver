...
module.exports = exports = function compileAndWriteStylus( compileOptions, docRoot, pathname, content, context ) {
    var filePath = path.join(docRoot, pathname.replace('.css', '.styl'));
    ...
    compileOptions.filename = filePath;
    ...
    style.render(function (err, css) {
        ...
        if ( err ) {
            ...
            if (style.evaluator && style.evaluator.requireHistory) {
                filecache.set(filePath, Object.keys(style.evaluator.requireHistory), css);
            }
